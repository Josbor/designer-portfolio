    ...
    <section id="experience" className="py-20 bg-white dark:bg-gray-800">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="font-bold text-3xl mb-2 text-gray-800 dark:text-white">{ translations.experience.title }</h2>
          <p className="text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">{ translations.experience.description }</p>
        </div>

        <div className="max-w-4xl mx-auto">
          { translations.experience.items.map( ( exp, index ) => (
            <div
              key={ index }
              className={ `flex flex-col md:flex-row gap-4 mb-10 relative ${ index !== translations.experience.items.length - 1 ? "pb-10" : ""
                }` }
            >
              {/* Timeline connector */ }
              { index !== translations.experience.items.length - 1 && (
              ...
              <div className="flex-1 bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                <h3 className="font-medium text-lg text-gray-800 dark:text-white">{ exp.title }</h3>
